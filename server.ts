...
app.post('/api/gemini/analyze', async (req, res) => {
  const { environmentDesc, orientation, layoutDesc } = req.body;

  if (!process.env.GEMINI_API_KEY) {
    return res.status(500).send('GEMINI_API_KEY is not configured in secrets.');
  }
...
`;
    ...
    res.status(500).send(err.message || 'Error occurred during analysis.');
  }
});
...
async function startServer() {
  ...
  app.listen(PORT, '0.0.0.0', () => {
    console.log(`[Feng Shui Backend] Server running on http://localhost:${PORT}`);
  });
}
